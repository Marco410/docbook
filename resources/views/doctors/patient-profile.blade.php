				...
				<div class="card widget-profile pat-widget-profile">
					<div class="card-body">
							...
							<div class="profile-info-widget">
								...
								<div class="profile-det-info">
									...
									<div class="patient-details">
										...
										<h5 class="mb-0"><i class="fas fa-map-marker-alt"></i> {{ $paciente->calle }} {{ $paciente->colonia }}, {{ $paciente->cp }}, {{ $paciente->estado }}, {{ $paciente->pais }}</h5>
									</div>
								</div>
							</div>
						</div>
						<div class="patient-info">
							<ul>
								<li>Teléfono <span>{{ $paciente->telefono }}</span></li>
								<li>Edad <span>{{ $paciente->get_edad() }} años, {{ $paciente->sexo }}</span></li>
								<li>Tipo Sangre <span>{{ $paciente->tipo_sangre }}</span></li>
							</ul>
						</div>
					</div>
				</div>
													...
													<tr>
														<td>
															<h2 class="table-avatar">
																...
																<a href="doctor-profile">Dr. {{ $consulta->doctor->nombre }} {{ $consulta->doctor->apellido_p }} {{ $consulta->doctor->apellido_m }}  <span>{{ $consulta->doctor->especialidad[0]->nombre }} </span></a>
															</h2>
														</td>
														<td><span class="text-primary" >{{ $consulta->motivo->motivo }}</span></td>
														<td>
															@if ($consulta->diagnostico_id)
																@if(strlen($consulta->diagnostico()->first()->descripcion_4) > 25)
																<span title="{{$consulta->diagnostico()->first()->descripcion_4}}" >{{ substr($consulta->diagnostico()->first()->descripcion_4, 0, 25) . "..." }}</span>

																@endif
															@else 
															@if(strlen($consulta->diagnostico_str) > 25)
																
																<span title="{{$consulta->diagnostico_str}}" >{{ substr($consulta->diagnostico_str, 0, 25) . "..." }}</span>

																@endif
															@endif
														</td>
														<td>{{ date("d M Y", $fechaInt) }} <span class="d-block text-info">{{ date("h:i A", $fechaInt) }}</span></td>
														<td>
